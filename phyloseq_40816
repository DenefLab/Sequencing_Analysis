...
#########################################################################################################################
#stacked bar plot to determine changes in community composition of Transect Sediment samples
sed_phylum <- Final %>%
...
ggplot(sed_phylum, aes(x = Month_Collected, y = Abundance, fill = Phylum)) + 
 # facet_grid(Station~.) +
...
###############################################################################################################
#Alpda Diversity
#calculate min of sample reads
min.lib <- min(sample_sums(Final))
#it comes out at 7 and that number is bad so we will use 1,500

# Initialize matrices to store richness and evenness estimates
nsamp = nsamples(Final)
trials = 100

richness <- matrix(nrow = nsamp, ncol = trials)
row.names(richness) <- sample_names(Final)

evenness <- matrix(nrow = nsamp, ncol = trials)
row.names(evenness) <- sample_names(Final)
...
for (i in 1:100) {
  # Subsample
  r <- rarefy_even_depth(Final, sample.size = 1500, verbose = FALSE, replace = TRUE)
