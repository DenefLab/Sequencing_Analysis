...
density <- subset_samples(metadata$Mussel_Density(levels=c("Very Few", "Few", "Few-Moderate",
                                                      "Moderate", "Moderate-high", "Many", "High")))
############################################################################################
...
#Scale the reads for "Final" by using code originally from Michelle Berry (Done in functions part of code
...
plot_ordination(
 ...
 # shape = "Mussel.Classification",
  ...
  geom_point(colour = "grey90", size = 1.5)
##########################################################
#PcoA of samples by mussel density
DensityPCoA <-
  scale_Final %>%
...
# Adonis test
adonis(final_bray ~ Duplicate, data = sampledf)
...
transect_sed <- subset_samples(scale_Final, Cruise == "Transect" & Type == "Sediment")

# Initialize matrices to store richness and evenness estimates
nsamp = nsamples(transect_sed)
trials = 100

richness <- matrix(nrow = nsamp, ncol = trials)
row.names(richness) <- sample_names(transect_sed)

evenness <- matrix(nrow = nsamp, ncol = trials)
row.names(evenness) <- sample_names(transect_sed)
...
for (i in 1:100) {
  # Subsample
  r <- rarefy_even_depth(transect_sed, sample.size = 2000, verbose = FALSE, replace = TRUE)
...
s <- data.frame(sample_data(transect_sed))
alphadiv <- merge(alpha, s, by.x="SampleID", by.y = "New_Name") 
...
ggplot(alphadiv, aes(x = Month_Collected, y = mean, color = Depth_in_Meters, group = Depth_in_Meters)) +
  ...
  scale_fill_manual(values = c("#E96446", "#302F3D", "#87CEFA", "red"), name = "Depth in Meters") +
